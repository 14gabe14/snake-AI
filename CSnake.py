...
class CSnake:
    ...
    def act(self, state):
        ...
        # EXPLOIT
            ...
            state = state.__array__() #copies state
            state = torch.tensor(state)
            state = state.unsqueeze(0) #changes size to (1, -, -, -)
    ...
    def sync_Q_target(self):
        #Sync online with target
        self.net.target.load_state_dict(self.net.online.state_dict())

    def save(self):
        save_path = (
            self.save_dir / f"snake_net_{int(self.curr_step // self.save_every)}.chkpt"
        )
        torch.save(
